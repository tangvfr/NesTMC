...
public class NES {
	...
	public static final String VERSION = "062-NesTMC";
    ...
    public void run() {
        ...
        while (!this.shutdown) {
            if (this.runEmulation) {
            	...
            	wait = (this.frameTime / 1_000_000) - ((System.nanoTime() - frameStartTime) / 1_000_000) - 1;
            	if (wait > 0) {
	                try {
						Thread.sleep(wait);
            	...
            	try {
            		Thread.sleep(this.frameTime / 1_000_000);
            	} catch (InterruptedException e) {
            		e.printStackTrace();
            	}
            	if (ppu != null && framecount > 1) {
    ...
    public synchronized void setParameters() {
        ...
        if (/*limiter != null && */mapper != null) {
            switch (mapper.getTVType()) {
	            case NTSC:
	            default:
	            	this.frameTime = 16639267;//60hz
	                //limiter.setInterval(16639267);
	                break;
	                
                case PAL:
                case DENDY:
                	this.frameTime = 19997200;//50hz
